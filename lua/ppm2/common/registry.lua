PPM2.ALLOW_TO_MODIFY_SCALE = CreateConVar('ppm2_sv_allow_resize', '1', {
  FCVAR_NOTIFY,
  FCVAR_REPLICATED
}, 'Allow to resize ponies. Disables resizing completely (visual; mechanical)')
...
for _, model in ipairs({
  ...
  player_manager.AddValidHands(model, 'models/cppm/pony_arms.mdl', 0, '')
end
PPM2.MIN_WEIGHT = 0.7
PPM2.MAX_WEIGHT = 1.5
PPM2.MIN_SCALE = 0.5
...
do
  local _accum_0 = { }
  local _len_0 = 1
  for _, mat in ipairs(PPM2.AvaliableLightwarps) do
    _accum_0[_len_0] = 'models/ppm2/lightwarps/' .. mat:lower()
    _len_0 = _len_0 + 1
  end
  PPM2.AvaliableLightwarpsPaths = _accum_0
end
...
do
  ...
  for _, mark in ipairs(PPM2.DefaultCutiemarks) do
    i = i + 1
    PPM2["CMARK_" .. tostring(mark:upper())] = i
  end
end
